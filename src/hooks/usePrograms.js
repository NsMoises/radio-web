...
import programsFallback from "../data/programs.json";

const API_URL = "/api/programs.php";
const LS_KEY = "radio-web:programs:backup";
const EV_KEY = "programs-saved";
...
export function usePrograms() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!json || !Array.isArray(json.programs)) throw new Error("Formato inválido");
      setData(json);
      try { localStorage.setItem(LS_KEY, JSON.stringify(json)); } catch {}
      return json;
    } catch {
      try { const ls = localStorage.getItem(LS_KEY); if (ls) { const j = JSON.parse(ls); if (j && Array.isArray(j.programs) && j.programs.length > 0) { setData(j); setError("offline"); return j; } } } catch {}
      setData(programsFallback); setError("offline"); return programsFallback;
