...
import djsFallback from "../data/djs.json";

const API_URL = "/api/djs.php";
const LS_KEY = "radio-web:djs:backup";
const EV_KEY = "djs-saved";
...
export function useDjs() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!json || !Array.isArray(json.djs)) throw new Error("Formato invÃ¡lido");
      setData(json);
      try { localStorage.setItem(LS_KEY, JSON.stringify(json)); } catch {}
      return json;
    } catch {
      try { const ls = localStorage.getItem(LS_KEY); if (ls) { const j = JSON.parse(ls); if (j && Array.isArray(j.djs) && j.djs.length > 0) { setData(j); setError("offline"); return j; } } } catch {}
      setData(djsFallback); setError("offline"); return djsFallback;
