  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (ls) { const j = JSON.parse(ls); if (j && Array.isArray(j.songs) && j.songs.length > 0) { setData(j); setError("offline"); return j; } }
      } catch {}
      setData(rankingFallback);
      setError("offline");
