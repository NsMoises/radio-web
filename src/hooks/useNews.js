...
const LS_KEY = "radio-web:news:backup";
const EV_KEY = "news-saved";
...
export function useNews() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!json || !Array.isArray(json.articles)) throw new Error("Formato invÃ¡lido");
      setData(json);
      try { localStorage.setItem(LS_KEY, JSON.stringify(json)); } catch {}
      return json;
    } catch {
      try { const ls = localStorage.getItem(LS_KEY); if (ls) { const j = JSON.parse(ls); if (j && Array.isArray(j.articles) && j.articles.length > 0) { setData(j); setError("offline"); return j; } } } catch {}
      setData(newsFallback); setError("offline"); return newsFallback;
