...
// Valida que el candidatoId exista realmente en la lista de candidatos.
function isCandInList($candidatoId, $rf) {
  if (!file_exists($rf)) return true;
  $rd = json_decode(file_get_contents($rf), true) ?: [];
  foreach (($rd["candidatos"] ?? []) as $s) {
    $vid = is_string($s["videoId"] ?? "") ? $s["videoId"] : "";
    if ($vid === "") $vid = extractVideoId($s["url"] ?? "");
    if ($vid !== "" && $vid === $candidatoId) return true;
  }
  return false;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // El candidatoId debe estar en la lista actual (anti-spam / items fuera de lista)
  $rf = DATA_DIR . "/candidatos.json";
  if (!isCandInList($candidatoId, $rf)) {
    http_response_code(400);
    echo json_encode(["ok" => false, "error" => "Ese candidato no está en la lista"]);
    exit;
  }
  ...
  if ($last > 0 && (time() - $last) < COOLDOWN_MIN) {
    ...
    echo json_encode(["ok" => false, "error" => "Espera unos segundos antes de votar de nuevo", "votes" => getCounts($list), "myVote" => myVote($list, $ip, $today)]);
    exit;
  }
