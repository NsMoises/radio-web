...
// Valida que el songId exista realmente en el ranking Top 20.
// Evita votos de items que salieron de la lista o IDs inventados.
function isSongInList($songId, $rf) {
  if (!file_exists($rf)) return true; // si no hay lista, no bloquear
  $rd = json_decode(file_get_contents($rf), true) ?: [];
  foreach (($rd["songs"] ?? []) as $s) {
    $vid = is_string($s["videoId"] ?? "") ? $s["videoId"] : "";
    if ($vid === "") $vid = extractVideoId($s["url"] ?? "");
    if ($vid !== "" && $vid === $songId) return true;
  }
  return false;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // El songId debe estar en la lista actual (anti-spam / items fuera de lista)
  $rf = DATA_DIR . "/ranking.json";
  if (!isSongInList($songId, $rf)) {
    http_response_code(400);
    echo json_encode(["ok" => false, "error" => "Esa canción no está en el Top 20"]);
    exit;
  }
