...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
  if (!file_exists(DATA_FILE)) {
    ...
    echo json_encode(["ok" => false, "error" => "top15videos.json no existe"]);
    exit;
  }
  readfile(DATA_FILE);
  exit;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($ok === false) {
    ...
    echo json_encode(["ok" => false, "error" => "No se pudo escribir data/top15videos.json (revisa permisos chmod 775)"]);
    exit;
  }
  echo json_encode(["ok" => true, "saved" => count($data["videos"]) . " vídeos"]);
  exit;
}
